...
router.post('/generate', async (req: EbookRequest, res: Response) => {
  try {
    ...
    if (!chart || !userEmail) {
      return res.status(400).json({ error: 'Chart e userEmail são obrigatórios' });
    }
    
    const pdfBuffer = await generateEbook(chart);
    
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="Guia_Numerologia_${chart.fullName.replace(/\s+/g, '_')}.pdf"`);
    res.send(pdfBuffer);
  } catch (error) {
    console.error('Erro ao gerar e-book:', error);
    res.status(500).json({ error: 'Erro ao gerar e-book' });
  }
});
